...
const GradientWaves = ({
  ...
  useEffect(() => {
    ...
    // 性能：这是全屏 raymarching，每个像素要跑 uSteps(70) 次 plasma。
    // dpr 2 时 1920x1080 屏 = 8.3M 像素 x 70 步 ≈ 5.8 亿次/帧，中低端 GPU 必掉帧。
    // 画面是低频渐变波浪，没有任何高频细节，dpr 1 视觉无损 → 像素量直接降到 1/4。
    ...
    // 注意：容器 .bg-waves 是 pointer-events:none（背景层不能挡住内容交互），
    // 所以监听挂在 canvas 上永远收不到事件 —— 视差会一直停在中心点。
    // 改挂 window，并用 rAF 合并高频 pointermove。
    let praf = 0
    let px = 0.5
    let py = 0.5
    ...
    // 限帧 30fps：波浪 speed 只有 0.12，30fps 与 60fps 肉眼无差，
    // 但 GPU 负载直接减半，跟逐帧滚动的合成工作错开。
